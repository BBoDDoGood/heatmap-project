...
def analyze():
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT DISTINCT time_window_start_sec FROM heatmaps WHERE video_id=%s", (vid,)
    )
...
def api_dashboard(vid):
    ...
    cur.execute(
        "SELECT COUNT(DISTINCT person_id) FROM trajectories WHERE video_id=%s", (vid,)
    )
    persons = cur.fetchone()[0]
    cur.execute(
        "SELECT x_grid, y_grid, SUM(count) FROM heatmaps WHERE video_id=%s GROUP BY x_grid,y_grid ORDER BY SUM(count) DESC LIMIT 5",
        (vid,),
    )
    top5 = [{"x": x, "y": y, "count": cnt} for x, y, cnt in cur.fetchall()]
...
def api_cell(vid, x, y):
    ...
    cur = conn.cursor()
    cur.execute(
        """SELECT time_window_start_sec, count
                   FROM heatmaps 
                   WHERE video_id=%s AND x_grid=%s AND y_grid=%s 
                   ORDER BY time_window_start_sec""",
        (vid, x, y),
    )
    series = []
    snaps = []
    for t, cnt in cur.fetchall():
        ...
        if os.path.exists(snap_path):
            snaps.append(url_for("static", filename=f"snaps/{snap_fn}"))
    conn.close()
